...
const Card: React.FC<CardProps> = ({ children, className = "", onClick, hover = false }) => {
    ...
    return (
        <div className={`rounded-md bg-white/50 dark:bg-secondary-950/80 border border-secondary-950/40 dark:border-white/10 transition-all ${hoverStyles} ${className}`} onClick={onClick}>
            {children}
        </div>
    );
};
